import numpy as np
from core.fuzzy_logic import FuzzyPID
...
class MIMOLQRController:
    """
    Full state-feedback LQR with Integral action (LQR+I) for the 2-DOF Helicopter.
    Uses the 12-parameter K matrix to compute u = -K * x_i.
    Error state vector: [e_p, e_y, e_dot_p, e_dot_y, int_e_p, int_e_y]
    """
    def __init__(self, params_12, setpoint_pitch=0.0, setpoint_yaw=0.0, v_limits=(-24.0, 24.0)):
        # K matrix is 2x6
        self.K = np.array(params_12).reshape(2, 6)
        self.setpoint_pitch = setpoint_pitch
        self.setpoint_yaw = setpoint_yaw
        self.output_limits = v_limits
    ...
    def compute(self, current_pitch, current_yaw, dt, setpoint_pitch=None, setpoint_yaw=None):
        if setpoint_pitch is not None:
            self.setpoint_pitch = setpoint_pitch
        if setpoint_yaw is not None:
            self.setpoint_yaw = setpoint_yaw
            
        e_p = self.setpoint_pitch - current_pitch
        e_y = self.setpoint_yaw - current_yaw
        
        # Integral
        self._integral_p += e_p * dt
        self._integral_y += e_y * dt
        
        # Derivative (velocity of error)
        e_dot_p = (e_p - self._prev_e_p) / dt if dt > 0 else 0.0
        e_dot_y = (e_y - self._prev_e_y) / dt if dt > 0 else 0.0
        
        self._prev_e_p = e_p
        self._prev_e_y = e_y
        
        # Error state vector x_e: [e_theta, e_dot_theta, e_psi, e_dot_psi, int_e_theta, int_e_psi]
        # Mạch LQR tính K dựa trên A = [theta, theta_dot, psi, psi_dot, int_theta, int_psi]
        x_e = np.array([e_p, e_dot_p, e_y, e_dot_y, self._integral_p, self._integral_y])
        
        # Control law: u = K * x_e
        u = self.K @ x_e
        v_pitch, v_yaw = u[0], u[1]
        ...
        return float(v_pitch), float(v_yaw)
