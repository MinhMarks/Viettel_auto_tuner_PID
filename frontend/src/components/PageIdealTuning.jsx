...
function ConvergenceChart({ compareResult, height = 300 }) {
    ...
    ['GA', 'PSO', 'BO'].forEach(alg => {
        if (compareResult[alg] && compareResult[alg].best_overall && compareResult[alg].best_overall.cost_history) {
            // Skip first 3 iterations to avoid initial high cost spikes ruining the chart scale
            const hist = compareResult[alg].best_overall.cost_history.slice(3); 
    ...
    const xAxisData = Array.from({length: maxIters}, (_, i) => i + 4);

    const option = {
        title: { text: 'Convergence Chart (Cost vs Iterations)', textStyle: { color: '#ffffff', fontSize: 14, fontWeight: 'bold' }, top: 0, left: 10 },
        tooltip: { trigger: 'axis' },
        legend: { data: series.map(s => s.name), textStyle: { color: '#ffffff', fontWeight: 'bold' }, top: '5%', right: '10%' },
    ...
    return (
        <div className="glass-panel" style={{padding: 16, display: 'flex', gap: 16, flexDirection: 'column'}}>
            <div style={{display:'flex', gap: 8, zIndex: 10, flexWrap: 'wrap'}}>
               {iterations.map(item => (
                   <button key={item.iters} 
                       ...
                       onClick={() => setVisibleIters({...visibleIters, [item.iters]: !visibleIters[item.iters]})} 
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: 16}}>
                <TimeResponseChart title={`${alg} Convergence`} dataSeries={seriesData} timeAxis={timeAxis} spPitchData={spPitchData} spYawData={spYawData} height={350} />
...
export default function PageIdealTuning({ 
    ...
    const [itersInput, setItersInput] = useState({ GA: "10, 20", PSO: "10, 20", BO: "10, 20" });
    ...
    const [visiblePlots, setVisiblePlots] = useState({ Manual: true, GA: true, PSO: false, BO: false, Target: true });
    ...
    const handleRun = async () => {
        ...
        try {
            // 1. Simulate Manual
            const manualRes = await axios.post(`${API_BASE}/simulate`, {
                params: manualParams,
                ...
                if (fullResults[alg]) {
                    ...
                    if (fullResults[alg].best_overall) {
                        const sRes = await axios.post(`${API_BASE}/simulate`, {
                            ...
                            t_max: simDuration, disturbance_config: disturbances, trajectory_type: trajectoryType, tuning_profile: tuningProfile
                        });
            ...
            if (onUpdateTunedParams) {
                ...
                if (fullResults.BO?.best_overall) newTuned.BO = fullResults.BO.best_overall.best_params;
    ...
    if (visiblePlots.BO && compareResult?.BO?.best_overall?.simData) mainSeries.push({ name: 'Best BO', data: compareResult.BO.best_overall.simData, colorP: '#ec4899', colorY: '#f9a8d4' });
            ...
            <div className="glass-panel" style={{padding: 16, display: 'flex', alignItems: 'center', gap: 16}}>
                <div style={{display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap'}}>
                    <div style={{flex: 1, minWidth: 200}}>
                        ...
                        <p style={{fontSize: 13, color: '#aaa', margin: 0}}>Compare Manual PID baseline against Auto-Tuned (GA, PSO, BO) controllers.</p>
                    </div>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', marginLeft: 10}}>
                            <label style={{fontSize: 12, color: '#aaa', display: 'flex', alignItems: 'center', gap: 4, zIndex: 999}}>
                                ...
                                <div style={{position: 'relative', display: 'inline-block'}} 
                                    ...
                                    <button 
                                        onClick={() => {
                                            if (setMainTab) setMainTab('wiki');
                                            if (setWikiDoc) setWikiDoc('objective_func');
                                            
                                            // Dùng cơ chế polling để đảm bảo DOM render xong markdown thì mới scroll
                                            let attempts = 0;
                                            const interval = setInterval(() => {
                                                ...
                                                attempts++;
                                                if (attempts > 20) clearInterval(interval); // Timeout sau 2 giây
                                            }, 100);
                                        }} 
                                        style={{
